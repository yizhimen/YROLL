...
def test_full_edit_flow_over_http(client: TestClient):
    # 初始为空工程
    assert client.get("/project").json()["clips"] == {}
    ...
    # trim / speed / volume
    assert client.post(f"/clips/{clip_id}/trim",
                       json={"new_source_start": 2.0}).status_code == 200
    assert client.post(f"/clips/{clip_id}/speed", json={"speed": 2.0}).status_code == 200
    assert client.post(f"/clips/{clip_id}/volume", json={"volume": 0.8}).status_code == 200
    ...
    # 语义化撤销
    vol_op = ops[-1]
    r = client.post("/revert", json={"operation_id": vol_op["operation_id"]})
    assert r.status_code == 200
    assert r.json()["type"] == "revert:volume"

    # 版本
    assert client.post("/versions", params={"note": "v1"}).status_code == 200
    assert len(client.get("/versions").json()) == 1


def test_errors(client: TestClient):
    r = client.post("/clips/bad-id/trim", json={"new_source_start": 1.0})
    assert r.status_code == 400
    r = client.post("/revert", json={"operation_id": "op99999"})
    assert r.status_code == 404


def test_gui_static_hosting(client: TestClient):
...
def test_import_asset_upload(client: TestClient, tmp_path: Path):
    ...
    r2 = client.post("/assets/import",
                     files={"file": ("clip.mp4", src.read_bytes(), "video/mp4")})
    assert r2.json()["deduped"] is True
    assert len(client.get("/project").json()["assets"]) == 1


def test_track_and_transform_api(client: TestClient):
    r = client.post("/tracks", params={"kind": "video", "track_id": "v2"})
    assert r.status_code == 200
    clip = client.post("/clips", json={
        "asset_id": "a1", "source_start": 0, "source_end": 5,
        "timeline_start": 0, "track_id": "v2"}).json()
    r2 = client.post(f"/clips/{clip['clip_id']}/transform",
                     json={"x": 0.1, "y": 0.1, "scale": 0.5})
    assert r2.status_code == 200
    proj = client.get("/project").json()
    assert proj["clips"][clip["clip_id"]]["transform"]["scale"] == 0.5
    # 撤销 transform
    client.post("/revert", json={"operation_id": r2.json()["operation_id"]})
    assert client.get("/project").json()["clips"][clip["clip_id"]]["transform"] == {}


def test_project_switch_and_chat_history(client: TestClient, tmp_path: Path):
    """多工程切换 + 会话历史持久化。"""
    # 新建第二个工程并切换
    r = client.post("/project/new", params={"root": str(tmp_path), "name": "proj-b", "goal": "测试"})
    assert r.status_code == 200
    assert client.get("/project").json()["name"] == "proj-b"
    # 会话历史写读
    client.post("/chat/history", params={"who": "user", "text": "你好"})
    client.post("/chat/history", params={"who": "ai", "text": "在的"})
    msgs = client.get("/chat/history").json()["messages"]
    assert [m["text"] for m in msgs] == ["你好", "在的"]
    # 历史落盘在工程目录
    assert (tmp_path / "proj-b" / "chat_log.json").exists()
    # 切回第一个工程
    first = client.get("/project").json()
    r2 = client.post("/project/open", params={"path": str(tmp_path / "api-demo")})
    assert r2.status_code == 200
    assert client.get("/project").json()["name"] == "api-demo"
    # 打开不存在的工程 → 404
    assert client.post("/project/open", params={"path": "D:/nope"}).status_code == 404


def test_import_ai_gen_sidecar(client: TestClient, tmp_path: Path):
